...
export class ConfigManager {
   ...
   * The memory server uses its own default store (memory.jsonl beside the
   * installed @modelcontextprotocol/server-memory package).
   *
  ...
  static syncProjectMcpServers(root: string, log?: (m: string) => void): void {
    ...
    try {
      ...
      if (origin && key && !disabledBuiltins.includes('pixel-office')) {
        // Both variants go through the `autodev` CLI stdio bridge to the OPERATOR
        // MCP (…/api/office-mcp — the mcp-operate default): the FULL toolkit
        // (get_tasks/start_task/complete_task/report/set_status/check_messages/…)
